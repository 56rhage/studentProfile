...
export default class ListOfStudents extends Component{
  ...
  render(){
      const {term, allStudents, currentPage, todosPerPage} = this.state;

      // Logic for displaying page numbers
      const pageNumbers = [];
      for (let i = 1; i <= Math.ceil(allStudents.filter(searchingFor(term)).length / todosPerPage); i++) {
        pageNumbers.push(i);
      }
      ...
      const currentTodos = allStudents.filter(searchingFor(term)).slice(indexOfFirstTodo, indexOfLastTodo);

      //Pagination Arrows
      const leftArrow = (currentPage - 1) < 1 ? "1" : String(currentPage - 1);
      const rightArrow = (currentPage + 1) > pageNumbers.length ? String(pageNumbers.length) : String(currentPage + 1);
      ...
      <div>
          ...
          <div className="w3-cell-content">
              <div className="w3-row w3-border">
                  {currentTodos.map((displayAllStudents, index) => {
